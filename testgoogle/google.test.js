...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys("Rubber duck", Key.RETURN);
  await driver.wait(until.titleIs("Rubber duck - Google Search"), 1000);
  ...
  await driver.findElement(By.name("q")).sendKeys("Real duck", Key.RETURN);

  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("Real duck - Google Search"), 1000);
});
